...
class MyController(http.Controller):
    ...
    @http.route('/fsms/twilio/receipt', type="http", auth="public")
    def sms_twilio_receipt(self, **kwargs):
        values = {}
	for field_name, field_value in kwargs.items():
            values[field_name] = field_value
        
        request.env['esms.twilio'].delivary_receipt(values['AccountSid'], values['MessageSid'])
        
        return "OK"        
        
    @http.route('/fsms/twilio/receive', type="http", auth="public")
